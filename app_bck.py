...
def send_email_with_retry(service, message, max_retries=5, initial_delay=2):
    """Enhanced email sending with exponential backoff"""
    for attempt in range(max_retries):
        try:
            ...
            if e.resp.status in [500, 503, 429, 408] and attempt < max_retries - 1:
                delay = initial_delay * (2 ** attempt)
                logger.warning(f"Attempt {attempt+1} failed for {message['To']}. Retrying in {delay}s...")
                time.sleep(delay)
                continue
            raise
            ...
            if attempt < max_retries - 1:
                delay = initial_delay * (2 ** attempt)
                logger.warning(f"General error on attempt {attempt+1}. Retrying in {delay}s...")
                time.sleep(delay)
                continue
            raise
